...
class PhysioDataset(Dataset):
    def __init__(self,
                 ...
                 plot=False, 
                 savepath='./figs',
                 apply_masking=False,  # New argument for masking
                 mask_ratio=0.15):      # New argument for mask ratio
        ...
        pprint.pprint(
            {
                ...
                "Masking Enabled": self.apply_masking,
    ...
    def _generate_mask_indices(self, seq_len):
        """
        Generates random masking indices for a given sequence length.
        Args:
            seq_len (int): The length of the sequence to mask.
        Returns:
            torch.Tensor: Boolean tensor indicating masked positions.
        """
        num_mask_tokens = int(self.mask_ratio * seq_len)
        
        # Randomly select indices to mask
        masked_indices_flat = torch.randperm(seq_len)[:num_mask_tokens]
        masked_indices = torch.zeros(seq_len, dtype=torch.bool)
        masked_indices[masked_indices_flat] = True
    ...
    def __getitem__(self, index):
        ...
        if self.ecg and not self.resp and not self.ppg_derivatives and not self.ppg_emd and not self.ppg_freqs:
            ...
            sample['sig'] = np.expand_dims(sample['ppg'], axis=-1) # Ensure it's 2D for consistent masking

        # Convert signals to torch tensor
        signals = torch.tensor(sample['sig'], dtype=torch.float32)
        
        masked_indices = torch.zeros(signals.shape[0], dtype=torch.bool) # Default to no masking

        if self.apply_masking:
            masked_indices = self._generate_mask_indices(signals.shape[0])
        ...
        if self.sig2sig:
            ...
            if self.apply_masking:
                return signals, abp, masked_indices
            else:
                return signals, abp
        else:
            ...
            sbp_val = torch.tensor(sample['sbp'], dtype=torch.float32).unsqueeze(-1)
            dbp_val = torch.tensor(sample['dbp'], dtype=torch.float32).unsqueeze(-1)
            
            if self.apply_masking:
                return signals, [sbp_val, dbp_val], masked_indices
            else:
...
def parseargs():
    ...
    parser.add_argument('--apply_masking', default='False', type=lambda x: bool(strtobool(x)), help='whether to apply masking for self-supervised learning') # New argument
    parser.add_argument('--mask_ratio', default=0.15, type=float, help='ratio of samples to mask when apply_masking is true') # New argument
...
if __name__ == "__main__":
    ...
    dataset = PhysioDataset(
        ...
        apply_masking=args.apply_masking, # Pass new argument
        mask_ratio=args.mask_ratio # Pass new argument
    )
    ...
    if args.apply_masking:
        sig, annotation, masked_indices_batch = input_batch
    else:
        sig, annotation = input_batch

    original_sig_for_plot = sig.clone().unsqueeze(-1) if len(sig.shape) == 2 else sig.clone()
    
    idx = np.random.randint(0, sig.shape[0])

    if args.sig2sig:
        original_sig = original_sig_for_plot[idx, :, :].squeeze().numpy()
        abp = annotation[idx, :].squeeze().numpy()
        if args.apply_masking:
            masked_sig_for_plot = original_sig_for_plot[idx, :, :].squeeze().clone()
            current_masked_indices = masked_indices_batch[idx]
            masked_sig_for_plot[current_masked_indices, :] = 0.0 # Manually apply mask for plotting
            masked_sig = masked_sig_for_plot.numpy()
    else:
    ...
    if args.ecg and not args.resp and not args.ppg_derivatives and not args.ppg_emd and not args.ppg_freqs:
        ...
        if args.sig2sig:
            sigs = np.concatenate((original_sig, abp[:, np.newaxis]), axis=1)
            plot_signals(
                sigs.T, 
                fs=args.fs, 
                labels=['PPG', 'ECG', 'ABP'], 
            ...
            if args.apply_masking:
                masked_sigs_plot = np.concatenate((masked_sig, abp[:, np.newaxis]), axis=1)
                plot_signals(
                    masked_sigs_plot.T, 
                    fs=args.fs, 
                    labels=['PPG (masked)', 'ECG (masked)', 'ABP'], 
                    title=f'Masked Input: PPG + ECG, Output: ABP (Mask Ratio: {args.mask_ratio*100:.0f}%)', 
                    savepath=root_figs_folder, 
                    ylabels=['a.u.', 'mV', 'mmHg']
                )
        else:
            plot_signals(
                original_sig.T, fs=args.fs, 
                labels=['PPG', 'ECG'], 
            ...
            if args.apply_masking:
                plot_signals(
                    masked_sig.T, fs=args.fs, 
                    labels=['PPG (masked)', 'ECG (masked)'], 
                    title=f'Masked Input: PPG + ECG, Output: [SBP {sbp_val:.2f} - DBP {dbp_val:.2f}] (Mask Ratio: {args.mask_ratio*100:.0f}%)', 
                    savepath=os.path.join(root_figs_folder, 'masked_ppg_ecg.jpg'), 
                    ylabels=['a.u.', 'mV']
                )
        ...
        if args.plot_aug:
            ...
            inp_sigs_augs = []
            for inp_mod in range(original_sig.shape[-1]):
                sig_aug = augments(original_sig[:, inp_mod].squeeze())
                sig_aug = sig_aug.unsqueeze(-1) if len(sig_aug.shape) == 1 else sig_aug.unsqueeze(-1) # Ensure 2D
        ...
        if args.sig2sig:
            ...
            if args.apply_masking:
                ...
                plot_signals(
                    masked_sigs_plot.T, 
                    fs=args.fs, 
                    labels=['PPG (masked)', 'ECG (masked)', 'RESP (masked)', 'ABP'], 
                    title=f'Masked Input: PPG + ECG + RESP, Output: ABP (Mask Ratio: {args.mask_ratio*100:.0f}%)', 
                    savepath=os.path.join(root_figs_folder, 'masked_ppg_ecg_resp_abp.jpg'), 
                    ylabels=['a.u.', 'mV', 'pm', 'mmHg']
                )
        else:    
            plot_signals(
            ...
            if args.apply_masking:
                plot_signals(
                    masked_sig.T, 
                    fs=args.fs, 
                    labels=['PPG (masked)', 'ECG (masked)', 'RESP (masked)'], 
                    title=f'Masked Input: PPG + ECG + RESP, Output: [SBP {sbp_val:.2f} - DBP {dbp_val:.2f}] (Mask Ratio: {args.mask_ratio*100:.0f}%)', 
                    savepath=os.path.join(root_figs_folder, 'masked_ppg_ecg_resp.jpg'), 
                    ylabels=['a.u.', 'mV', 'pm']
                )
        ...
        if args.plot_aug:
            ...
            for inp_mod in range(original_sig.shape[-1]):
                sig_aug = augments(original_sig[:, inp_mod].squeeze())
                sig_aug = sig_aug.unsqueeze(-1) if len(sig_aug.shape) == 1 else sig_aug.unsqueeze(-1)
                inp_sigs_augs.append(sig_aug)
            inp_sigs_augs = torch.cat(inp_sigs_augs, dim=-1)
            plot_signals(
                inp_sigs_augs.T, 
        ...
        if args.sig2sig:
            ...
            if args.apply_masking:
                ...
                plot_signals(
                    masked_sigs_plot.T, 
                    fs=args.fs, 
                    labels=['PPG (masked)', 'VPG (masked)', 'APG (masked)', 'ABP'], 
                    title=f'Masked Input: PPG + VPG + APG, Output: ABP (Mask Ratio: {args.mask_ratio*100:.0f}%)', 
                    savepath=os.path.join(root_figs_folder, 'masked_ppg_vpg_apg_abp.jpg'), 
                    ylabels=['a.u.', 'a.u.', 'a.u.', 'mmHg']
                )
        else:    
            plot_signals(
            ...
            if args.apply_masking:
                plot_signals(
                    masked_sig.T,
                    fs=args.fs, 
                    labels=['PPG (masked)', 'VPG (masked)', 'APG (masked)'], 
                    title=f'Masked Input: PPG + VPG + APG, Output: [SBP {sbp_val:.2f} - DBP {dbp_val:.2f}] (Mask Ratio: {args.mask_ratio*100:.0f}%)', 
                    savepath=os.path.join(root_figs_folder, 'masked_ppg_vpg_apg.jpg'), 
                    ylabels=['a.u.', 'a.u.', 'a.u.']
                )
        ...
        if args.sig2sig:
            ...
            if args.apply_masking:
                ...
                plot_signals(
                    masked_sigs_plot.T, 
                    fs=args.fs, 
                    labels=['IMF0 (masked)', 'IMF1 (masked)', 'IMF2 (masked)', 'IMF3 (masked)', 'ABP'], 
                    title=f'Masked Input: PPG EMD (4 channels), Output: ABP (Mask Ratio: {args.mask_ratio*100:.0f}%)', 
                    savepath=os.path.join(root_figs_folder, 'masked_ppg_emd_abp.jpg'), 
                    ylabels=['a.u.', 'a.u.', 'a.u.', 'a.u.', 'mmHg']
                )
        else:
            plot_signals(
            ...
            if args.apply_masking:
                plot_signals(
                    masked_sig.T,
                    fs=args.fs, 
                    labels=['IMF0 (masked)', 'IMF1 (masked)', 'IMF2 (masked)', 'IMF3 (masked)'], 
                    title=f'Masked Input: PPG EMD (4 channels), Output: [SBP {sbp_val:.2f} - DBP {dbp_val:.2f}] (Mask Ratio: {args.mask_ratio*100:.0f}%)', 
                    savepath=os.path.join(root_figs_folder, 'masked_ppg_emd.jpg'), 
                    ylabels=['a.u.', 'a.u.', 'a.u.', 'a.u.']
                )
        ...
        if args.apply_masking:
            print('Plot masked scalogram to be done soon ...')
    else: # Only PPG
        ...
        if args.sig2sig:
            ...
            if args.apply_masking:
                ...
                plot_signals(
                    masked_sigs_plot.T, 
                    fs=args.fs, 
                    labels=['PPG (masked)', 'ABP'], 
                    title=f'Masked Input: PPG, Output: ABP (Mask Ratio: {args.mask_ratio*100:.0f}%)', 
                    savepath=os.path.join(root_figs_folder, 'masked_ppg_abp.jpg'), 
                    ylabels=['a.u.', 'mmHg']
                )
        else:
            plot_signals(
            ...
            if args.apply_masking:
                plot_signals(
                    ...
                    fs=args.fs,
                    labels=['PPG (masked)'], 
                    title=f'Masked Input: PPG, Ouput: [SBP {sbp_val:.2f} - DBP {dbp_val:.2f}] (Mask Ratio: {args.mask_ratio*100:.0f}%)', 
                    savepath=os.path.join(root_figs_folder, 'masked_ppg.jpg'), 
                    ylabels=['a.u.']
                )
