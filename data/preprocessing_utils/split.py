...
def split_train_val_test(subject_sample_ids, split_ratio, shuffle=False):
    ...
    Parameters
    ------------
        subject_sample_ids (list): 
            List of sample IDs to be split.
        split_ratio (list): 
            List of three floats representing the desired ratios for train, validation, and test sets.
        shuffle (bool, optional): 
            Whether to shuffle the sample IDs before splitting. Defaults to True.

    Returns
    ------------
        tuple: 
            A tuple containing three lists: train_idx, val_idx, and test_idx.
    """
...
def split_train_val_test_personalization(subject_sample_ids, fixed_train_size, fixed_val_size=50, fixed_test_size=70):
    ...
    ------------
        ...
        fixed_train_size (int): 
            Fixed number of samples for the training set (at the beginning of subject_sample_ids).
        fixed_val_size (int): 
            Fixed number of samples for the validation set (before the test set at the end of remaining).
        fixed_test_size (int): 
            Fixed number of samples for the test set (from the end of remaining).

    Returns
    ------------
        tuple: 
            A tuple containing three lists: personalization_idx, val_idx, and test_idx.
    """
