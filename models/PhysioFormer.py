...
class Transformer(nn.Module):
    def __init__(self, num_modalities=1, embed_dim=768, n_head=12, head_dim=64, hidden_dim=3072, num_layers=3, input_seq_len=625):
        ...
        self.pos_encoder = PositionalEncoding(embed_dim=embed_dim * num_modalities, max_len=(input_seq_len + 1)) # +1 for the cls_token
        self.cls_token = nn.Parameter(torch.randn(1, 1, embed_dim * num_modalities))
        self.transformer_encoder = nn.Sequential(*[AttentionBlock(embed_dim=embed_dim * num_modalities, hidden_dim=hidden_dim, num_heads=n_head, head_dim=head_dim) for _ in range(num_layers)])
...
class PhysioFormer(nn.Module):
    def __init__(self, 
        ...
        self.ppg_linear_projection = nn.Linear(ppg_in_channels, embed_dim) 
        
        if self.ecg:
            self.ecg_linear_projection = nn.Linear(ecg_in_channels, embed_dim)
            
            if self.resp:
                self.resp_linear_projection = nn.Linear(resp_in_channels, embed_dim)

        # PPG is always present while ECG/RESP may not int(ecg) == 0 when false, same for resp of course
        self.transformer = Transformer(num_modalities=1 + int(self.ecg) + int(self.resp), embed_dim=embed_dim, n_head=n_head, head_dim=head_dim, hidden_dim=hidden_dim, num_layers=num_layers, input_seq_len=input_seq_len)
        
        # Projection Head
        if self.ecg and not self.resp: 
            self.projection_head = nn.Linear(embed_dim * 2, proj_head_dim)
        elif self.ecg and self.resp: 
            self.projection_head = nn.Linear(embed_dim * 3, proj_head_dim)
        else:
            self.projection_head = nn.Linear(embed_dim, proj_head_dim)
    ...
    def forward(self, x):
        ...
        # x -> [batch_size, seq_len, channels]
        if self.ecg and not self.resp:
            ppg_t = self.ppg_linear_projection(x[:,:,:-1])
            ecg_t = self.ecg_linear_projection(x[:,:,-1].unsqueeze(-1)) # slicing squeeze dimension automatically
        elif self.ecg and self.resp:
            ppg_t = self.ppg_linear_projection(x[:,:,:-2])
            ecg_t = self.ecg_linear_projection(x[:,:,-2].unsqueeze(-1)) # slicing squeeze dimension automatically
            resp_t = self.resp_linear_projection(x[:,:,-1].unsqueeze(-1)) # slicing squeeze dimension automatically
        else:
            ppg_t = self.ppg_linear_projection(x)
        
        # Get cls token instead of flatten
        if self.ecg and not self.resp:
            feature = torch.cat((
                ppg_t, 
                ecg_t
        ...
        feature = self.transformer(feature) # feature -> (batch_size, seq_len, embed_dim)
        
        # Get CLS token
        feature = feature[:, 0, :]
        
        # Projection head
        embedding = self.projection_head(feature)
