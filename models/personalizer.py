...
from component_factory import ReservoirReplayBuffer, FeatureDriftDetector, Model, RidgeHead
...
from data.preprocessing_utils.data_visualization import plot_blockwise_mae, plot_subject_annotation_blocks, plot_update_summary_table, plot_param_updates
...
def eval_model_on_sample_set(encoder, prediction_head, dataset, sample_list, device, config):
    ...
    if not config['use_ridge']:
        # Set eval only when using MLP head, since Ridge is not trained with backprop and does not have dropout/batchnorm layers
...
def personalize_no_adapt(baseline, dataset, subject_id, subj_dir, writer, device, config):
    ...
    # -> evaluation now is prequential Test-Time-Adaption
...
def personalize_first_batch_finetune(baseline, dataset, subject_id, subj_dir, writer, device, config):
    r"""
    ...
    for batch_info in dataset.get_subject_blocks(
        ...
        if not first_finetuned and do_adapt:
            ...
            # Divide features and targets into training/validation
            split_idx = int(0.75 * features.shape[0])
            ...
            best_ph_state = ph.state_dict()  # Initialize with current state in case no improvement
            ...
            if best_ph_state is not None:
                ...
                raise ValueError("Best model state is None, check adaptation function for issues.")
               
            # NOTE: feature drift detector updated with the full batch features and not only the training ones
            # Update detector statistics only after adaptation
    ...
    for batch_info in dataset.get_subject_blocks(
        ...
        if do_adapt:
            ...
            opt = build_inner_optimizer(
                ...
                adapted_head=ph, 
                base_lr=config['personalization_lr'], 
                mode='all', # this baseline is from scratch
                opt_type=config['inner_opt'].lower(), 
                config=config
            )
...
def adapt_ridge_from_encoder(train_features, train_targets, val_features, val_targets, enc, ph, replay_buffer, step_idx, config, device):
    
    # Train the ridge regression on the encoder features
    # -> ph is assumed to be of type RidgeHead
    ...
    param_update_logging_info = {
        "step_idx": step_idx,
        "update_mode": config['inner_adapt'],
        "n_updated_params": n_updated,
        "total_params": encoder_params + n_updated,
        "fraction_updated": n_updated / (encoder_params + n_updated)
    }

    return best_state, param_update_logging_info


def adapt_mlp_from_encoder(train_features, train_targets, val_features, val_targets, enc, ph, replay_buffer, step_idx, config, device):
    
    # Train the mlp head on the encoder features
    # -> ph is assumed to be of type RidgeHead
    ...
    return best_ph_state, param_update_logging_info
...
def personalize_feature_replay(baseline, dataset, subject_id, subj_dir, writer, device, config):
    ...
    # To device
    enc = pretrained_learner.encoder.to(device)
    ph = pretrained_learner.prediction_head.to(device) if not config['use_ridge'] else RidgeHead(in_dim=enc.embed_dim, out_dim=3).to(device)
    
    if config['use_ridge']:
        stats = torch.load(config['pretraining_feats_stats'], map_location=device)
        
        # Manual loading because W, mean and std are not registered as buffers/parameters in the RidgeHead class
        ph.W = stats["W"]
        ph.mean = stats["mean"]
        ph.std = stats["std"]
        print(f"[Personalization] Ridge Head pre-trained ckpt loaded ✓")
    ...
    for batch_info in dataset.get_subject_blocks(
        ...
        if do_adapt:
            ...
            if config['use_ridge']:
                best_ph_state, param_update_logging_info = adapt_ridge_from_encoder(
                    train_features, train_targets, val_features, val_targets, enc, ph, replay_buffer, step_idx, config, device
                )
            else:
                best_ph_state, param_update_logging_info = adapt_mlp_from_encoder(
                    train_features, train_targets, val_features, val_targets, enc, ph, replay_buffer, step_idx, config, device
                )
            ...
            # Log the parameter update info
            param_update_log.append(param_update_logging_info)

            # NOTE: feature drift detector and replay buffer updated with the full batch features and not only the training ones
            # Update detector statistics only after adaptation
            feature_drift_detector.update_stats(features)
